...
export class NodeFileSystem implements FileSystem {
  isDirectory(dirPath: string): boolean {
    ...
    return fs.existsSync(crossPlatformPath) && fs.statSync(crossPlatformPath).isDirectory();
  }

  isFile(filePath: string): boolean {
    const crossPlatformPath = this.toCrossPlatformPath(filePath);
    return fs.existsSync(crossPlatformPath) && fs.statSync(crossPlatformPath).isFile();
  }
  ...
  getFilesByRegExpPatern(dirPath: string, pattern: string, allFiles: string[] = [], recurse: boolean = true): string[] {
    ...
    for (const file of files) {
        ...
        if (stats.isDirectory()) {
            if (recurse) {
                ...
                this.getFilesByRegExpPatern(filePath, pattern, allFiles, recurse);
            }
  ...
  matchesPattern(filePath: string, pattern: string): boolean {
    ...
    const regexPattern = new RegExp('^' + baseNamePattern.replace(/\*/g, '.*') + '$'); // Convert wildcard to regex
    const fileName = path.basename(filePath);

    return regexPattern.test(fileName);
  }

  getGlobFilesSize(pathPatern: string): bigint {
    const rootDir = this.extractRootFolder(pathPatern);
    const shouldRecurse = this.shouldRecurse(pathPatern);

    const files = this.getFilesByRegExpPatern(rootDir, pathPatern, [], shouldRecurse)
    let totalSize = BigInt(0);
